...
*                                                                                                          *;
* Note: this is a clean version computed analytical SE without bootstrapping                                                                                                         *;
*----------------------------------------------------------------------------------------------------------*;
 ...
 *---------------------------------------------------------------------------------------------------------*;
   ...
   *-------------------------------------------------------------------------------------------------------------*;
	...
	*-------------------------------------------*;
	...
	proc sql;
		...
		select &subject., &tdctime., %do i = 1 %to &L.; &&Var&i , %end; &t1., &t2., _censor_,
     ...
     *-------------------------------------------------------------*;
		...
		select &SUBJECT. ,&TDCTIME. %do i = %eval(&N.+1) %to &L.; ,&&Var&i %end;
		from UseData
		;
     *-----------------------------------------------------------------------------------------------*;
     ...
     *-----------------------------------------------------------------------------------------------*;
		...
		select DISTINCT &SUBJECT. ,&T1. ,&T2. , _CENSOR_ %do i = 1 %to &N.; ,&&Var&i %end; ,K
		from UseData
		;
	 quit;
     data SUBJECT;
	...
	proc sort data=usedata;by &subject &tdctime;run;
   ...
   *--------------------*;
   PROC IML;   ** WORKSIZE=250000;
    ...
    /*-------------------------------------------------------------*/
     Use SUBJECT;                         /* one row per patient   */
      read all var{&SUBJECT} into ID;     /* Subject ID numbers    */
     ...
     *---------------------------------------------------*;
     Use TDCVARS;
      read all var{&SUBJECT &TDCTIME &TDC} into TDC;
      read all var{&SUBJECT} into ID_DUPS;
     Close TDCVARS;
     *-------------------------------------------------------*;
    ...
    *---------------------------------------------------------*;
     *-------------------------------------------------------------*;
     * *** Compute D_n: sum of weighted squared first derivative ***;
     * The argument W to the function is the parameter vector at   *;
     * which the likelihood is calculated.                         *;
     * The argument is assumed partitioned as follows:             *;
     * w(|1|)=alpha                                                *;
     ...
     * w(|p+4...p+q+3|)=eta                                        *;
     *-------------------------------------------------------------*;
     START DSUM(W) GLOBAL(N,P,Q,R,Z,K,T,TDC,CENSOR,START,STOP,EPSILON);
        Grad  = J(r,1,0);              /* Initialize Gradient to 0 */
        Alpha = w(|1|);                /* Extract a few things to  */
        Theta = w(|2|);                /* simplify the code.       */
        Kappa = w(|3|);
			...
			END; ** End of k(|i|) > 1 do loop **;
               ...
               /*------------------------------------------------*/
                if Cens=0 then do;
                  ...
                  *--- Gradient --*;
                    Gradient = Gradient + ( Grad_i - Grad1 );
                    DSUM = DSUM + ( Grad_i - Grad1 )`*( Grad_i - Grad1 );
                end;
                if Cens=1 then do;
					S2 = exp(-capL2);
					if 1-S2 ^= 0 
						then DSUM = DSUM + ((S2/(1-S2))#Grad2)`*((S2/(1-S2))#Grad2);
						else DSUM = DSUM;
                end;
                if Cens=2 then do;
                    DSUM = DSUM + (Grad1/&prob)`*(Grad1/&prob); * for gradient, it was a negative sign;
					/*Gradient = Gradient/&prob; /*weighted*/
                end;
                if Cens=3 then do;
					S1 = exp(-capL1);
					S2 = exp(-capL2);
					if S1-S2 ^= 0 
						then DSUM = DSUM+(((S2#Grad2)-(S1#Grad1))/(S1-S2))`*(((S2#Grad2)-(S1#Grad1))/(S1-S2));
						else DSUM = DSUM;
                end;
               /*--------------------------------------------------*/
               /* End computation Gradient Computation.            */
       ...
       *--------------------------------------------------------------------------------------*;
          ...
          *--------------------*;
             Info = -H;
		  	 D_N = DSUM(omega);
		     newVAR = inv(H)*D_N*inv(H);
/*		     newVAR = solve(Info,I(r))*D_N*solve(Info,I(r));*/
     %END;
      /*------------------------------------------------------------*/
...
/*			  D_N=Premult*D_N*Postmult;*/
			  newVAR_ord=Premult*newVAR*Postmult;
          *-----------------------------------*;
          ...
          *-----------------------------------*;
           PostMult=({1}//J(r-1,1,0))||(J(2,r-2,0)//I(r-2))||
            ...
            PreMult=({1}//J(r-1,1,0))||(J(r-1,1,0)//{1})||
			  ...
			  Info=Premult*Info*Postmult;
/*			  D_N=Premult*D_N*Postmult;*/
/*			  solve_info=solve(Info,I(r));*/
/*			  inv_info=inv(Info,I(r));*/
/*		     newVAR_ord = solve(Info,I(r))*D_N*solve(Info,I(r));*/
			  newVAR_ord=Premult*newVAR_ord*Postmult;
      /*------------------------------------------------------------*/
      ...
      /*------------------------------------------------------------*/
		%IF %LENGTH(&FIXED_KAPPA.)^=0 %THEN %DO;
			%IF %LENGTH(&FIXED_SCALE.)^=0 %THEN %DO;
               Covpar = inv(Info(|1:r-2,1:r-2|));
             SE_Parms = sqrt(vecdiag(CovPar));
			 SE_sigma = sqrt(vecdiag(newVAR_ord(|1:r-2,1:r-2|)));/*closed form*/
		/* remove elements related to alpha and kappa when they are fixed*/
			  H_sub = (H(|2,2|)//H(|4:r,2|))||
					(H(|2,4:r|)//H(|4:r,4:r|));
			  D_N_sub = (D_N(|2,2|)//D_N(|4:r,2|))||
						(D_N(|2,4:r|)//D_N(|4:r,4:r|));
			  newVAR_sub = inv(H_sub)*D_N_sub*inv(H_sub);
				SE_sigma_sub = sqrt(vecdiag(newVAR_sub));/*closed form*/
              Low95CI = Parms(|1:r-2|)-(1.96#SE_sigma_sub);
             High95CI = Parms(|1:r-2|)+(1.96#SE_sigma_sub);
              Wald_X2 = (Parms(|1:r-2|)##2)/(SE_sigma_sub##2);
               Wald_P = 1-Probchi(Wald_X2,1);
             SE_sigma_sub = SE_sigma_sub // {0,0};
              Low95CI = Low95CI  // {.,.};
		     High95CI = High95CI // {.,.};
              Wald_X2 = Wald_X2  // {.,.};
               Wald_P = Wald_P   // {.,.};
			%END;
			%ELSE %DO;
               Covpar = inv(Info(|1:r-1,1:r-1|));
             SE_Parms = sqrt(vecdiag(CovPar));
	          *----------------------*;
	          * Move Alpha to bottom *;
	          *----------------------*;
	           PostMult=(J(1,r-2,0)||{1}   ) // (I(r-2)||J(r-2,1,0));
	            PreMult=(J(r-2,1,0)||I(r-2)) // (   {1}||J(1,r-2,0));
/*remove elements related to kappa when it is fixed*/
			  H_sub = (H(|1:2,1:2|)//H(|4:r,1:2|))||
					(H(|1:2,4:r|)//H(|4:r,4:r|));
			  D_N_sub = (D_N(|1:2,1:2|)//D_N(|4:r,1:2|))||
						(D_N(|1:2,4:r|)//D_N(|4:r,4:r|));
			  newVAR_sub = inv(H_sub)*D_N_sub*inv(H_sub);
				  newVAR_sub_ord=Premult*newVAR_sub*Postmult;
				SE_sigma_sub = sqrt(vecdiag(newVAR_sub_ord));

              Low95CI = Parms(|1:r-1|)-(1.96#SE_sigma_sub);
             High95CI = Parms(|1:r-1|)+(1.96#SE_sigma_sub);
			 SE_sigma = sqrt(vecdiag(newVAR_ord(|1:r-1,1:r-1|)));/*closed form*/
              Wald_X2 = (Parms(|1:r-1|)##2)/(SE_sigma_sub##2);
               Wald_P = 1-Probchi(Wald_X2,1);
             SE_sigma_sub = SE_sigma_sub // {0};
              Low95CI = Low95CI  // {.};
		     High95CI = High95CI // {.};
              Wald_X2 = Wald_X2  // {.};
               Wald_P = Wald_P   // {.};
		...
		%ELSE %DO;
            Covpar = inv(Info);
          SE_Parms = sqrt(vecdiag(CovPar));
		  SE_sigma = sqrt(vecdiag(newVAR_ord));
			SE_sigma_sub = SE_sigma;/*closed form*/
           Low95CI = Parms-(1.96#SE_sigma_sub);
          High95CI = Parms+(1.96#SE_sigma_sub);
		  Parms_Ho = J(r-1,1,0)//{&kappa_Ho.};
           Wald_X2 = ((Parms-Parms_Ho)##2)/(SE_sigma_sub##2);
            Wald_P = 1-Probchi(Wald_X2,1);
      ...
      /*-----------------------*/
      %IF %LENGTH(&FIXED_KAPPA.)^=0 %THEN %DO;
			Covpar = ( Covpar || J(r-1,1,0) ) // J(1,r,0) ;
      %END;
      Estimate=Parms; SE=SE_sigma_sub;
      Create &SAVELIB..COVPARMS FROM CovPar(|colname=CNames|);
         append from CovPar; close &SAVELIB..COVPARMS;
      Create &SAVELIB..LRTMODEL VAR {N NObs LogLNull LogL df
                                     LRT_X2 LRT_P };
         append VAR {N NObs LogLNull LogL df LRT_X2 LRT_P };
         close &SAVELIB..LRTMODEL;
      Create &SAVELIB..PARMEST
         VAR {Parameter Estimate SE Low95CI High95CI Wald_X2 Wald_P};
      append VAR {Parameter Estimate SE Low95CI High95CI Wald_X2 Wald_P};
         close &SAVELIB..PARMEST;
   QUIT;
   data &SAVELIB..COVPARMS;
      merge &SAVELIB..PARMEST(Keep=Parameter) &SAVELIB..COVPARMS;
   run;
   *----------------------------------------------------------*;
   ...
   *------------------------------*;
    ...
    run;
    data _LRT;
       set &SAVELIB..LRTMODEL; LLCat=1; output; LRTCat=2; output;
    run;
    data _PARMEST;
       set &SAVELIB..PARMEST;
       Low95CI=ROUND(Low95CI,0.0001);
       High95CI=ROUND(High95CI,0.0001);
       Wald_X2=ROUND(Wald_X2,0.0001);
       CICat=3;   output;
       WaldCat=4; output;
    run;
	data ds_out;
	  ...
	  newpage:
        put  // @13 "*" +1 85*"-" @100 "*"
              / @13 "*" @15 "Case-cohort interval-censored data with time-dependent covariates"
						@100 "*"
			  / @13 "*" @20 "by Gao X., Hudgens M., and Zou, F. - (2022+)"
						@100 "*"
   ...
   *-------------------------------------------------------------*;
	proc datasets library=work memtype=data nolist;
		delete InputDat ReducedData Check_T0 censored_int censored_oth dummy1 dummy2 Recs_Added
			   UseData newuse TDCVARS SUBJECT INITPARM _LRT _PARMEST ds_out;
	quit;
%MEND;
